...
window_state() {
    ...
    WINDOW=$(aerospace query --windows --window)
    ...
    if [ "$(echo "$WINDOW" | jq '.["is-floating"]')" = "true" ]; then
        ...
        LAST_STACK_INDEX=$(aerospace query --windows --window stack.last | jq '.["stack-index"]')
        ICON+=$YABAI_STACK
...
"aerospace_window_focus")
    window_state
    ;;
"aerospace_windows_on_spaces" | "aerospace_space_change")
    windows_on_spaces
    ;;
esac
